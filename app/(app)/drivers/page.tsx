import { DriversWorkspace } from "@/components/drivers/drivers-workspace"
import type { PermissionAction } from "@/db/schema/constants"
import { requireSession } from "@/lib/auth-guard"
import { getUserPermissions } from "@/modules/rbac/rbac.service"

export default async function DriversPage() {
  const session = await requireSession()
  const permissions = await getUserPermissions(session.user.id)

  const has = (action: PermissionAction) =>
    permissions.some((p) => p.module === "drivers" && p.action === action)

  if (!has("view")) {
    return (
      <div className="text-sm text-muted-foreground">
        You don&apos;t have permission to view driver management.
      </div>
    )
  }

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-foreground">
          Drivers &amp; Safety
        </h1>
        <p className="text-sm text-muted-foreground">
          Manage driver availability, licensing, and safety compliance.
        </p>
      </div>
      <DriversWorkspace
        canCreate={has("create")}
        canEdit={has("edit")}
        canDelete={has("delete")}
      />
    </div>
  )
}
